  ...
  asyncHandler(async (req, res) => {
    ...
    try {
      ...
      if (
        ...
        return res.json({
          success: false,
          message: "Số tiền mua tối thiểu không được đáp ứng.",
        });
      }
      ...
      const isValid = products.every((product) => {
        ...
        if (
          coupon.applicableProduct &&
          coupon.applicableProduct._id.toString() !== product._id.toString()
        ) {
          return false;
        }
      ...
      if (isValid) {
        return res.json({
          success: true,
          message:
            "Phiếu giảm giá được áp dụng cho các sản phẩm được cung cấp.",
          data: coupon,
        });
      } else {
        return res.json({
          success: false,
          message:
            "Phiếu giảm giá không áp dụng cho các sản phẩm được cung cấp.",
        });
      }
    } catch (error) {
